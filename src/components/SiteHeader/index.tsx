...
function getPageTitle(pathname: string): string {
  ...
  const sortedSectionKeys = Object.keys(sectionTitles).sort(
    (a, b) => b.length - a.length
  );
  for (const sectionBasePath of sortedSectionKeys) {
    if (
      pathname.startsWith(sectionBasePath + "/") ||
  ...
  const pathSegmentsLower = pathname
    .toLowerCase()
    .split("/")
    .filter((segment) => segment !== "");
  const sortedFeatureKeywords = Object.keys(featureKeywordTitles).sort(
    (a, b) => b.length - a.length
  );
  ...
  const pathSegmentsOriginalCase = pathname
    .split("/")
    .filter((segment) => segment !== "");
  ...
  if (
    !lastSignificantSegment &&
    pathSegmentsOriginalCase.length > 1 &&
    pathSegmentsOriginalCase[pathSegmentsOriginalCase.length - 1].startsWith(
      "["
    )
  ) {
    const potentialSegment =
      pathSegmentsOriginalCase[pathSegmentsOriginalCase.length - 2];
  ...
  if (
    !lastSignificantSegment &&
    pathSegmentsOriginalCase.some((p) => p.startsWith("["))
  ) {
    if (pathSegmentsLower.length > 1) {
      const parentSegmentLower =
        pathSegmentsLower[pathSegmentsLower.length - 2];
      for (const keyword of sortedFeatureKeywords) {
  ...
  if (autoGeneratedTitle) {
    if (autoGeneratedTitle === "Beranda") {
      if (
        ...
        pathname.startsWith("/admin/beranda")
      ) {
        return "Beranda";
      }
    } else {
